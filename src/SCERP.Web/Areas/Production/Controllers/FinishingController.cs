...
namespace SCERP.Web.Areas.Production.Controllers
{
    public class FinishingController : BaseController
    {
        ...
        public FinishingController(IMachineManager machineManager,IProcessorManager processorManager, IProductionManager productionManager, IProgramManager programManager)
        {
        ...
        public ActionResult Index(ProductionViewModel model)
        {
            ModelState.Clear();
            var totalRecords = 0;
            model.VwPrograms = _programManager.GetVwProgramsByPaging(ProcessType.FINISHING, out totalRecords);
            model.TotalRecords = totalRecords;
            return View(model);
        }
        public ActionResult FinishingList(ProductionViewModel model)
        {
            ModelState.Clear();
            model.ProcessRefId = ProcessType.FINISHING;
            model.PType = "O";
            ...
            return PartialView("_FinishingList", model);
        }
        public ActionResult FInEdit(ProductionViewModel model)
        {
            ModelState.Clear();
            model.Production.ProcessRefId = ProcessType.FINISHING;
            ...
            else
            {
                ...
                model.Production.ProductionRefId = _productionManager.GetProductionRefId(ReferencePrefix.FininshingProduction);
        ...
        public ActionResult FOutEdit(ProductionViewModel model)
        {
            ModelState.Clear();
            model.Production.ProcessRefId = ProcessType.FINISHING;
        ...
        public ActionResult Save(ProductionViewModel model)
        {
            ...
            try
            {
                ...
                if (saveIndex > 0)
                {
                    ...
                    {
                        case "I":
                            return RedirectToAction("FInList", new { ProrgramRefId = model.Production.ProrgramRefId });
                        case "O":
                            return RedirectToAction("FOutList", new { ProrgramRefId = model.Production.ProrgramRefId });
                    }
        ...
        public ActionResult FInList(ProductionViewModel model)
        {
            ModelState.Clear();
            model.ProcessRefId = ProcessType.FINISHING;
            model.PType = "I";
            model.InProdProductions = _productionManager.GetProductionByPaging(model);
            return PartialView("_FInList", model);
        }

        public ActionResult FOutList(ProductionViewModel model)
        {
            ModelState.Clear();
            model.ProcessRefId = ProcessType.FINISHING;
            model.PType = "O";
            model.OutProdProductions = _productionManager.GetProductionByPaging(model);
            return PartialView("_FOutList", model);
        }
        ...
        public ActionResult Delete(ProductionViewModel model)
        {
            ...
            try
            {
                if (delete > 0)
                {
                    ...
                    {
                        case "I":
                            return RedirectToAction("FInList", new { ProrgramRefId = model.ProrgramRefId });
                        case "O":
                            return RedirectToAction("FOutList", new { ProrgramRefId = model.ProrgramRefId });
                    }
                }
                else
                {
