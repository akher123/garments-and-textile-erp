...
{
    public class PolyFinishingController : BaseController
    {
        ...
        public PolyFinishingController(IFinishingProcessManager finishingProcessManager, IOmBuyerManager buyerManager, IHourManager hourManager)
        {
            _finishingProcessManager = finishingProcessManager;
            _buyerManager = buyerManager;
            _hourManager = hourManager;
        }
        [AjaxAuthorize(Roles = "polyfinishing-1,polyfinishing-2,polyfinishing-3")]
        public ActionResult Index(FinishingProcessViewModel model)
        {
            ModelState.Clear();
            const string prifix = "PL";
            model.Buyers = _buyerManager.GetCuttingProcessStyleActiveBuyers() as IEnumerable;
            model.FinishingProcess.FinishingProcessRefId = _finishingProcessManager.GetNewFinishingProcessRefId((int)FinishType.Poly, prifix);
            return View(model);
        }
           [AjaxAuthorize(Roles = "polyfinishing-2,polyfinishing-3")]
        public ActionResult Find([Bind(Include = "FinishingProcess")] FinishingProcessViewModel model)
        {
            ...
            if (model.FinishingProcess.FinishingProcessId > 0)
            {
                ...
                model.VwFinishingProcessDetails = _finishingProcessManager.GetPolyFinishingProcessDetailByStyleColor(model.FinishingProcess.OrderStyleRefId, model.FinishingProcess.ColorRefId, model.FinishingProcess.FinishingProcessId);
                model.VwFinishingProcess = _finishingProcessManager.GetSewingInputProcessByStyleColor(model.FinishingProcess.OrderStyleRefId, model.FinishingProcess.ColorRefId, (int)FinishType.Poly);
            }
            else
            {
                ...
                {
                    FinishingProcessRefId = _finishingProcessManager.GetNewFinishingProcessRefId((int)FinishType.Poly, prifix),
            ...
            List<string> ttlSwOutQtyList = model.VwFinishingProcessDetails.Select(x => Convert.ToString(x.TinQuantity)).ToList();
            ttlSwOutQtyList.Add(Convert.ToString(model.VwFinishingProcessDetails.Sum(x => x.TinQuantity)));
            ...
            model.PivotDictionary.Add("IronQty", swoutputQtyList);
            model.PivotDictionary.Add("TotalPoly", ttlSwOutQtyList);
            model.PivotDictionary.Add("Poly %", inputPercentList);
            ...
            return PartialView("~/Areas/Production/Views/PolyFinishing/_Plate.cshtml", model);
        }
        [AjaxAuthorize(Roles = "polyfinishing-2,polyfinishing-3")]
        public ActionResult Save(FinishingProcessViewModel model)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (inputValue > 0)
                    {
                        model.FinishingProcess.FinishingProcessRefId =
                            _finishingProcessManager.GetNewFinishingProcessRefId((int) FinishType.Poly, prifix);
                        model.FinishingProcess.FType = (int)FinishType.Poly;
                ...
                if (index > 0)
                {
                    ...
                    List<string> swoutputQtyList = model.VwFinishingProcessDetails.Select(x => Convert.ToString(x.TtlSwOutQty)).ToList();
                    swoutputQtyList.Add(Convert.ToString(model.VwFinishingProcessDetails.Sum(x => x.TtlSwOutQty)));
                    List<string> ttlTinQuantityList = model.VwFinishingProcessDetails.Select(x => Convert.ToString(x.TinQuantity)).ToList();
                    ttlTinQuantityList.Add(Convert.ToString(model.VwFinishingProcessDetails.Sum(x => x.TinQuantity)));
                    List<string> inputPercentList = model.VwFinishingProcessDetails.Select(x => x.TinQuantity > 0 ? String.Format("{0:0.00}" + " " + "%", (x.TinQuantity * 100.00m) / x.TtlSwOutQty) : "0").ToList();
                    inputPercentList.Add(String.Format("{0:0.00}" + " " + "%", model.VwFinishingProcessDetails.Sum(x => x.TinQuantity) * 100.0m / model.VwFinishingProcessDetails.Sum(x => x.TtlSwOutQty)));
                    model.PivotDictionary.Add("Size", sizeList);
                    model.PivotDictionary.Add("OrderQty", ttlOrderQtyList);
                    model.PivotDictionary.Add("SewingInQty", swInputQtyList);
                    model.PivotDictionary.Add("SewingOutQty", swoutputQtyList);
                    model.PivotDictionary.Add("IronQty", swoutputQtyList);
                    model.PivotDictionary.Add("TotalPoly", ttlTinQuantityList);
                    model.PivotDictionary.Add("Input %", inputPercentList);
                    model.FinishingProcessDetailDictionary = new Dictionary<string, PROD_FinishingProcessDetail>();
                    foreach (var finishProces in model.VwFinishingProcessDetails)
                    {
                        model.FinishingProcessDetailDictionary.Add(finishProces.SizeRefId,new PROD_FinishingProcessDetail() { SizeRefId = finishProces.SizeRefId, CompId = PortalContext.CurrentUser.CompId });
                    }
                    model.VwFinishingProcess = _finishingProcessManager.GetSewingInputProcessByStyleColor(model.FinishingProcess.OrderStyleRefId, model.FinishingProcess.ColorRefId, (int)FinishType.Poly);
                    model.FinishingProcess = new PROD_FinishingProcess { FinishingProcessRefId = _finishingProcessManager.GetNewFinishingProcessRefId((int)FinishType.Poly, prifix),InputDate = DateTime.Now}; 
                    string partialViewString = RenderViewToString("~/Areas/Production/Views/PolyFinishing/_Plate.cshtml", model);
                    return Json(
                            new
                            {
                                SaveStatus = true,
            ...
            catch (Exception exception)
            {
                ...
                return ErrorResult("Fail to Save/Edit :");
                   ...
                   [AjaxAuthorize(Roles = "polyfinishing-3")]
        public ActionResult Delete(long finishingProcessId)
        {
            ...
            return deleted > 0 ? Reload() : ErrorResult("Failed To Delete Poly Finishing Process.");
        }

        [AjaxAuthorize(Roles = "dailyployfinishing-1,dailyployfinishing-2,dailyployfinishing-3")]
        public ActionResult DailyPolyFinishing(FinishingProcessViewModel model)
        {
            ModelState.Clear();
        ...
        public ActionResult DailyIronFinishingReport(FinishingProcessViewModel model)
        {
            ...
            {
                new ReportParameter("TitleName", "Daily Poly Finishing Report")
            };
