...
{
    public class DyesChemicalRegisterController : BaseController
    {
        ...
        public DyesChemicalRegisterController(IMaterialReceivedManager materialReceivedManager, IMateraialReceivedDetailManager materaialReceivedDetailManager)
        {
            _materialReceivedManager = materialReceivedManager;
            _materaialReceivedDetailManager = materaialReceivedDetailManager;
        }
        [AjaxAuthorize(Roles = "dyeschemicalregister-1,dyeschemicalregister-2,dyeschemicalregister-3")]
        public ActionResult Index(MaterialReceivedViewModel model)
        {
            try
            {
                ModelState.Clear();
                var totalRecords = 0;
                model.MaterialReceivedList = _materialReceivedManager.GetMaterialReceivedByPaging(model.FromDate, model.ToDate, model.SearchString, Convert.ToString(RegisterType.DYESCHEMICALREGISTER), PortalContext.CurrentUser.CompId, model.PageIndex, model.sort, model.sortdir, out totalRecords);
       ...
       [AjaxAuthorize(Roles = "dyeschemicalregister-2,dyeschemicalregister-3")]
        ...
        public ActionResult AddNewRow([Bind(Include = "MaterialReceivedDetail")]MaterialReceivedViewModel model)
        {
            ...
            return PartialView("~/Areas/Inventory/Views/DyesChemicalRegister/_AddNewRow.cshtml", model);
        }
        [AjaxAuthorize(Roles = "dyeschemicalregister-2,dyeschemicalregister-3")]
        public ActionResult Save(MaterialReceivedViewModel model)
        {
            ...
            try
            {
                ...
                {
                    model.MaterialReceived.RegisterType = Convert.ToString(RegisterType.DYESCHEMICALREGISTER);
                    ...
                    {
                        index = model.MaterialReceived.MaterialReceivedId > 0 ? _materialReceivedManager.EditMaterialReceived(model.MaterialReceived) : _materialReceivedManager.SaveMaterialReceived(model.MaterialReceived);
                    }
                }
        ...
        [AjaxAuthorize(Roles = "dyeschemicalregister-3")]
