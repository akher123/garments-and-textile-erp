...
{
    public class GreyRegisterController : BaseController
    {
        ...
        public GreyRegisterController(IMaterialReceivedManager materialReceivedManager, IMateraialReceivedDetailManager materaialReceivedDetailManager)
        {
            _materialReceivedManager = materialReceivedManager;
            _materaialReceivedDetailManager = materaialReceivedDetailManager;
        }
        [AjaxAuthorize(Roles = "greyfabregister-1,greyfabregister-2,greyfabregister-3")]
        public ActionResult Index(MaterialReceivedViewModel model)
        {
            try
            {
                ModelState.Clear();
                var totalRecords = 0;
                model.MaterialReceivedList = _materialReceivedManager.GetMaterialReceivedByPaging(model.FromDate, model.ToDate, model.SearchString, Convert.ToString(RegisterType.GREYFABRIC), PortalContext.CurrentUser.CompId, model.PageIndex, model.sort, model.sortdir, out totalRecords);
          ...
          [AjaxAuthorize(Roles = "greyfabregister-2,greyfabregister-3")]
        ...
        public ActionResult AddNewRow([Bind(Include = "MaterialReceivedDetail")]MaterialReceivedViewModel model)
        {
            ...
            return PartialView("~/Areas/Inventory/Views/GreyRegister/_AddNewRow.cshtml", model);
        }
           [AjaxAuthorize(Roles = "greyfabregister-2,greyfabregister-3")]
        public ActionResult Save(MaterialReceivedViewModel model)
        {
            ...
            try
            {
                ...
                {
                    model.MaterialReceived.RegisterType = Convert.ToString(RegisterType.GREYFABRIC);
        ...
        [AjaxAuthorize(Roles = "greyfabregister-3")]
