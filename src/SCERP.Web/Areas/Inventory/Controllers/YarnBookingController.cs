...
namespace SCERP.Web.Areas.Inventory.Controllers
{
    public class YarnBookingController : BaseController
    {
        ...
        public YarnBookingController(ISupplierCompanyManager supplierCompanyManager, IBookingManager bookingManager, IOmBuyerManager omBuyerManager, IMerchandiserManager merchandiserManager)
        {
        ...
        [AjaxAuthorize(Roles = "yarnbooking-1,yarnbooking-2,yarnbooking-3")]
        public ActionResult Index(BookingViewModel model)
        {
            ModelState.Clear();
            int totalRecords = 0;
            model.Booking.StoreId = (int)StoreType.Yarn;
        ...
        [AjaxAuthorize(Roles = "yarnbooking-2,yarnbooking-3")]
        public ActionResult Edit(BookingViewModel model)
        {
            ...
            {
                model.Booking.BookingRefId = _bookingManager.GetNewBookingRefId();
        ...
        [AjaxAuthorize(Roles = "yarnbooking-3")]
        ...
        public ActionResult BookingPdfReport(long bookingId)
        {
            ...
            string path = Path.Combine(Server.MapPath("~/Areas/Inventory/Report"), "YarnBookingReport.rdlc");
        ...
        public ActionResult ReceiveBalanceReport()
        {
            List<VwBookingDetailReport> bookingDetail = _bookingManager.GetVwBookingDetaliReport((int)StoreType.Yarn);
            string path = Path.Combine(Server.MapPath("~/Areas/Inventory/Report"), "ReceiveBalanceReport.rdlc");
            if (!System.IO.File.Exists(path))
            {
                return PartialView("~/Views/Shared/Error.cshtml");
            }
            var reportDataSources = new List<ReportDataSource>() { new ReportDataSource("BookingReceivedBalanceDSet", bookingDetail) };
            var deviceInformation = new DeviceInformation() { OutputFormat = 2, PageWidth = 11.69, PageHeight = 8.27, MarginTop = .2, MarginLeft = 0.5, MarginRight = 0.5, MarginBottom = .2 };
        ...
        {
            var storeId = (int)StoreType.Yarn;
            List<VwBookingSummaryReport> bookingDetail = _bookingManager.GetVwBookingDetalSummary(model.FromDate, model.ToDate, storeId);
