...
{
    public class YarnRegisterController : BaseController
    {
        ...
        public YarnRegisterController(IMaterialReceivedManager materialReceivedManager, IMateraialReceivedDetailManager materaialReceivedDetailManager)
        {
            _materialReceivedManager = materialReceivedManager;
            _materaialReceivedDetailManager = materaialReceivedDetailManager;
        }
        [AjaxAuthorize(Roles = "yarnregiser-1,yarnregiser-2,yarnregiser-3")]
        public ActionResult Index(MaterialReceivedViewModel model)
        {
            try
            {
                ModelState.Clear();
                var totalRecords = 0;
                model.MaterialReceivedList = _materialReceivedManager.GetMaterialReceivedByPaging(model.FromDate, model.ToDate, model.SearchString, Convert.ToString(RegisterType.YARNREGISTER), PortalContext.CurrentUser.CompId, model.PageIndex, model.sort, model.sortdir, out totalRecords);
          ...
          [AjaxAuthorize(Roles = "yarnregiser-2,yarnregiser-3")]
        ...
        public ActionResult AddNewRow([Bind(Include = "MaterialReceivedDetail")]MaterialReceivedViewModel model)
        {
            ...
            return PartialView("~/Areas/Inventory/Views/YarnRegister/_AddNewRow.cshtml", model);
        }
           [AjaxAuthorize(Roles = "yarnregiser-2,yarnregiser-3")]
        public ActionResult Save(MaterialReceivedViewModel model)
        {
            ...
            try
            {
                ...
                {
                    model.MaterialReceived.RegisterType = Convert.ToString(RegisterType.YARNREGISTER);
           ...
           [AjaxAuthorize(Roles = "yarnregiser-3")]
