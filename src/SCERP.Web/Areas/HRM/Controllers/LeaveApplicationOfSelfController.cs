...
{
    public class LeaveApplicationOfSelfController : BaseHrmController
    {
        private readonly int _pageSize = AppConfig.PageSize;
        private Guid? _employeeGuidId = PortalContext.CurrentUser.UserId;

        [AjaxAuthorize(Roles = "self-1,self-2,self-3")]
        public ActionResult Index(EmployeeLeaveViewModel model)
        {
            ModelState.Clear();
            EmployeeLeave employeeLeave = model;
            if (_employeeGuidId != null) employeeLeave.EmployeeId = _employeeGuidId.Value;
        ...
        [AjaxAuthorize(Roles = "self-2,self-3")]
        public ActionResult Edit(EmployeeLeaveViewModel model)
        {
            ...
            try
            {
                ...
                var approvalEmployeeList = EmployeeLeaveManager.GetAuthorizedPersons((int)SCERP.Common.ProcessKeyEnum.HRM_Leave, Convert.ToInt32(SCERP.Common.AuthorizationType.LeaveApproval));
                ...
                {
                    if (_employeeGuidId != null) model.EmployeeCardId = EmployeeManager.GetEmployeeById(_employeeGuidId.Value).EmployeeCardId;
