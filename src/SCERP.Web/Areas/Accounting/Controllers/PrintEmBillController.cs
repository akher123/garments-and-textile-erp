...
namespace SCERP.Web.Areas.Accounting.Controllers
{
    ...
    public class PrintEmBillController : BaseController
    {
        ...
        public PrintEmBillController(IJournalVoucherEntryManager journalVoucherEntryManager, IVoucherMasterManager voucherMasterManager, IAccountIntegrationManager accountIntegrationManager)
        {
        ...
        public ActionResult Index(PrintEmBillViewModel model)
        {
            ModelState.Clear();
            model.PrintEmbBills = _accountIntegrationManager.GetPrintEmbBillInvoice(PortalContext.CurrentUser.CompId,model.ProcessRefId);
            return View(model);
        }


        public ActionResult PrintEmBillVoucher(VoucherEntryViewModel model)
        {
            ModelState.Clear();
            ...
            Acc_VoucherMaster voucherMaster = _accountIntegrationManager.GetPrintEmbBillVoucher(model.Key, (int)BillTable.PROD_ProcessReceive);
            ...
            model.TotalAmountInWord = Spell.SpellAmount.InWrods(model.TotalDebitAmount);
