...
function getGradeByEmployeeTypeIed(employeeTypeId) {
    ...
    if (employeeTypeId != "") {
        $.getJSON('/ExceptionDay/GetGradeByGradeId', { id: employeeTypeId }).done(function (data) {
            if (data.length != 0) {
...
$('input.ExceptionDay').on('change', function () {
    $('input.ExceptionDay').not(this).prop('checked', false);
    var id = $(this).attr('id');
      
    var ischecked = $(this).is(':checked');
    if (ischecked) {
        $('.DeclaredDay').prop('disabled', false);
        $('#DeclaredDay-' + id).prop('disabled', true);
    }
    else
    {
        $('#DeclaredDay-' + id).prop('disabled', false);
    }
        
});

$('input.DeclaredDay').on('change', function () {
    $('input.DeclaredDay').not(this).prop('checked', false);
    var id = $(this).attr('id').split('-')[1];

    var ischecked = $(this).is(':checked');
    if (ischecked) {
        $('.ExceptionDay').prop('disabled', false);
