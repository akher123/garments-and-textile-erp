...
$('.UserMerchandiserEmployeeId').unbind("change").bind("change", changeEventHendeler);
    ...
    var option = {
        ...
        $.ajax(option).done(function (response) {
            if (response.length > 0) {
                uncheckedCheckBox();
                $.each(response, function (i, userEmployee) {
                    checkCheckBox(userEmployee);
                });
            } else {
                uncheckedCheckBox();
            }
...
function checkCheckBox(data) {
    ...
    $('#check-' + data.MerchandiserRefId).prop('checked', true);
