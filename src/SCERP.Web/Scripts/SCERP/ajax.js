function AjaxResponse(data, XMLHttpRequest) {
    var ajaxResponse = { IsHtml: false, IsJavaScript: false, IsJson: false, Content: data, Success: null, Message: null, HasError: false, Reload: false,PopupReload:false,PopupReloadUrl:null };
    ...
    if (jQuery.IsContentHtml(XMLHttpRequest)) {
        ...
        if (ajaxResponse.Reload) {
            if ((requseturl != undefined && requseturl != "")) {
                var formClssName = requseturl.split('/')[2] + 'SearchForm';
                search(requseturl,formClssName);
            }
      
        }
        if (ajaxResponse.PopupReload) {
            $('.ui-dialog-content').dialog('close');
            var action = ajaxResponse.PopupReloadUrl;
            jQuery.Ajax({
                url: action
                , type: "GET"
                , dialog: {}
...
var SCERPAjaxOptions = new function () {
            ...
            var ajaxRequest = $.ajax(url, {
                ...
                //, url: url
                 ...
                 , success: function (data, textStatus, XMLHttpRequest) {
                     //Newly added for showing pop-up
                     if (data.Success != 'undefined' && data.Success == false) {
                         if (data.Message != 'undefined' && data.Message.length > 0) {
                             if (data.Error == true) {
                                 ...
                                 document.title = requseturl.split('/')[2];
                                 alert(data.Message);
                             }
                             return false;
                         }
                             ...
                             var dialogOptions = jQuery.extend(
                                 {
                                     ...
                                     , resizable: false
                                     , draggable: true
                                     , hide: SCERPAjaxOptions.Dialog.Hide
                                     , show: SCERPAjaxOptions.Dialog.Show
