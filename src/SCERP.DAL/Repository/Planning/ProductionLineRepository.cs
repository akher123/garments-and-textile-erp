...
namespace SCERP.DAL.Repository.PlanningRepository
{
    public class ProductionLineRepository : Repository<PLAN_ProductionLine>, IProductionLineRepository
    {
        public ProductionLineRepository(SCERPDBContext context)
            : base(context)
        {
        }

        public List<PLAN_ProductionLine> GetProductionLine(int startPage, int pageSize, out int totalRecords, PLAN_ProductionLine model, SearchFieldModel searchFieldModel)
        {
            IQueryable<PLAN_ProductionLine> productionLines;
            try
            {
                productionLines =
                Context.PLAN_ProductionLines.Include(x => x.BranchUnitDepartment.BranchUnit.Branch.Company).Include(x => x.BranchUnitDepartment.BranchUnit.Unit).Include(x => x.BranchUnitDepartment.UnitDepartment.Department)
        ...
        public PLAN_ProductionLine GetProductionLineById(int productionLineId)
        {
            ...
            {
                productionLine =
                    Context.PLAN_ProductionLines.Include(x => x.BranchUnitDepartment.BranchUnit.Branch.Company)
                    .Include(x => x.BranchUnitDepartment.BranchUnit.Unit)
                        .Include(x => x.BranchUnitDepartment.UnitDepartment.Department)
                        .SingleOrDefault(x => x.ProductionLineId == productionLineId);
            }
            catch (Exception exception)
            {
        ...
        public List<PLAN_ProductionLine> GetProductionLineByBranchUnitDepartmentId(int branchUnitDepartmentId)
        {
            ...
            try
            {
                ...
                productionLines = (
                    Context.PLAN_ProductionLines.Include(x => x.Line)
                        .Where(x => x.BranchUnitDepartmentId == branchUnitDepartmentId && x.IsActive).ToList()
                        .Select(x => new PLAN_ProductionLine()
                        {
                            DisplayMember = x.Line.Name,
                            ValueMember = x.ProductionLineId
