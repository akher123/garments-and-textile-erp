...
{
    public class ContraVoucherEntryRepository : Repository<Acc_VoucherMaster>, IContraVoucherEntryRepository
    {
        public ContraVoucherEntryRepository(SCERPDBContext context)
            : base(context)
        {
        ...
        public List<string> GetAccountNames(string accountType)
        {
            var temp = from p in Context.Acc_GLAccounts
                where p.IsActive == true && p.AccountType == accountType
                select p;
        ...
        //{
        //    decimal? voucherLimit = 0;

        //    voucherLimit =
        //        Context.Acc_VoucherLimit.FirstOrDefault(p => p.VoucherType == VoucherType).VoucherLimit;

        //    if (voucherLimit < Convert.ToDecimal(Amount))
        //        return false;
        //    else
        //        return true;
        //}
        ...
        public string GetClosingBalance(Acc_ClosingBalanceViewModel obj)
        {
            ...
            voucherLimit = Context.Acc_VoucherLimit.FirstOrDefault(p => p.VoucherType == obj.VoucherType).VoucherLimit;

            if (voucherLimit < Convert.ToDecimal(obj.Amount))
                return "1";


            decimal? balance = 0;

            Acc_OpeningClosing OpenObj = Context.Acc_OpeningClosing.FirstOrDefault(p => p.SectorId == obj.SectorId
                                                                                        && p.FpId == obj.FpId && p.GlId == obj.GlId
                                                                                        && p.IsActive == true);
                                       
            if (OpenObj != null)
                balance = OpenObj.Debit - OpenObj.Credit;

            var temp =  from p in Context.Acc_VoucherMaster
                        join q in Context.Acc_VoucherDetail
                        on p.Id equals q.RefId
                        where p.SectorId == obj.SectorId && p.FinancialPeriodId == obj.FpId && p.VoucherType == "CB"
                        && q.GLID == obj.GlId && p.VoucherDate <= obj.StartDate && p.IsActive == true                       
                        select new
                        {
                            debit = q.Debit,
                            credit = q.Credit
            ...
            if (Convert.ToDecimal(obj.Amount) > balance)
                return "2";
