...
{
    public class CashVoucherEntryRepository : Repository<Acc_VoucherMaster>, ICashVoucherEntryRepository
    {
        public CashVoucherEntryRepository(SCERPDBContext context)
            : base(context)
        {
        ...
        public List<string> GetAccountNames()
        {
            var temp = from p in Context.Acc_GLAccounts
                where p.IsActive == true && p.AccountType != "CASH"
                select p;
        ...
        {
            var temp = (from p in Context.Acc_GLAccounts
                        where p.IsActive == true && p.AccountType == "CASH" && p.AccountName.Contains("Cash in Hand")
        ...
        public string GetClosingBalance(Acc_ClosingBalanceViewModel obj)
        {
            ...
            var temp = from p in Context.Acc_VoucherMaster
                       ...
                       where p.SectorId == obj.SectorId && p.FinancialPeriodId == obj.FpId
        ...
        {
            string AccName = "cash in hand";
            return Context.Acc_GLAccounts.FirstOrDefault(p => AccName.Contains(p.AccountName.ToLower())).Id;
        }
    }
}
