CREATE view [dbo].[DyeingAccountInvoice]
as
select ST.AccountName,ST.DglId,ST.InvoiceDate,ST.InvoiceNo,ST.Party,SUM(ST.Qty) AS Qty,ROUND(SUM(ST.Qty*ST.Rate)/SUM(ST.Qty),2) AS Rate,SUM(ST.Qty*ST.Rate) AS BillAmount from (
select 
...
group by ST.AccountName,ST.DglId,ST.InvoiceDate,ST.InvoiceNo,ST.Party
