...
-- Description:	<> EXEC [SPProcessEmployeeJobCard_10PM_NoWeekend]   2019, 08, '2019-08-01', '2019-08-31', ''
-- ===========================================================================================================

CREATE PROCEDURE [dbo].[SPProcessEmployeeJobCard_10PM_NoWeekend]
...
BEGIN
		...
		BEGIN	
							...
							DELETE FROM [EmployeeJobCard_10PM_NoWeekend] 
							WHERE Year = @Year 
							AND Month = @Month
							...
							INSERT INTO [dbo].[EmployeeJobCard_10PM_NoWeekend]
							   ([EmployeeId]
							   ,[Year]
							   ,[Month]
							   ,[MonthName]
						...
						SELECT [EmployeeId]
						  ...
						  SET TotalExtraOTHours = ISNULL((SELECT SUM([ExtraOTHours]) FROM [dbo].[EmployeeInOut_10PM_NoWeekend] WHERE [EmployeeInOut_10PM_NoWeekend].EmployeeId = [EmployeeJobCard_10PM_NoWeekend].EmployeeId AND CAST(TransactionDate AS DATE) BETWEEN @FromDate AND @ToDate GROUP BY EmployeeId), 0)		
						  WHERE Year = @Year 
						  AND Month = @Month
						  AND CAST(fromDate AS DATE) = @FromDate
						  AND CAST(toDate AS DATE) = @ToDate
