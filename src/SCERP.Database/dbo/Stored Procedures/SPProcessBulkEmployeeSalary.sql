...
-- Description:	<> EXEC [SPProcessBulkEmployeeSalary] -1, -1, -1, -1, -1, -1, -1,'', 2015, 10, '2015-10-01', '2015-10-31', 'superadmin'
-- =================================================================================================================================

CREATE PROCEDURE [dbo].[SPProcessBulkEmployeeSalary]
	   @CompanyId INT = -1,
	   @BranchId INT = -1,
	   @BranchUnitId INT = -1,
...
BEGIN
			...
			BEGIN
				...
				DELETE FROM [dbo].[EmployeeProcessedSalary]
				WHERE           
				(
				...
				BEGIN
					INSERT INTO [dbo].[EmployeeProcessedSalary]
							   ([EmployeeId]
							   ,[EmployeeCardId]
							   ,[Name]
							   ,[NameInBengali]
					...
					SELECT  ejc.EmployeeId
						   ...
						   ,CONVERT(DECIMAL(18,2), (ejc.GrossSalary - (ISNULL(ejc.LWPFee,0.0) + ISNULL(ejc.AbsentFee,0.0) + ISNULL(ejc.TotalPenaltyAbsentFee, 0.0) + ISNULL(ejc.TotalPenaltyFinancialAmount, 0.0) 
																	 ...
																	 + ejc.AttendanceBonus
																	 + ejc.ShiftingBonus))  AS TotalPaid    -- TotalPaid = gross - deduction + att Bonus + Shift Allow
						   ,ejc.OTRate AS Rate
						   ...
						   ,CASE WHEN ((ISNULL(ejc.TotalOTHours, 0.0)) >= (ISNULL(ejc.TotalPenaltyOTHours, 0.0))) 
								 THEN (CONVERT(DECIMAL(18,2), (ISNULL(ejc.TotalExtraOTHours, 0.0))))
							 ...
							 ,CASE WHEN ((ISNULL(ejc.TotalOTHours, 0.0)) >= (ISNULL(ejc.TotalPenaltyOTHours, 0.0))) 
								 THEN (CONVERT(DECIMAL(18,2), (ISNULL(ejc.TotalExtraOTAmount, 0.0))))
