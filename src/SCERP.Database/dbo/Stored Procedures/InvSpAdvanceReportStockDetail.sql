--exec [InvSpAdvanceReportStockDetail] 23,19,'2016-02-01','2016-06-16'
CREATE PROCEDURE [dbo].[InvSpAdvanceReportStockDetail]
...
INSERT INTO Inventory_RStock
                      (ItemId, ItemCode, OQty, OAmt, RQty, RAmt, IQty, IAmt,SizeRefId,ColorRefId,BrandId)
select distinct   SR.ItemId, ItemCode, 0 AS OQty, 0 AS OAmt, 0 AS RQty, 0 AS RAmt, 0 AS IQty, 0 AS IAmt,SR.SizeRefId,SR.ColorRefId,C.ColorCode as BrandId from Inventory_MaterialReceiveAgainstPo as MR
...
where SR.StoreId=1  and SR.ActionType=3 and (Sg.SubGroupId = @SubGroupId or @SubGroupId=-1)and g.GroupId=@GroupId and I.ItemType=1
...
select R.ItemId,R.ItemCode, R.OQty, R.OAmt, R.RQty, R.RAmt, R.IQty,IAmt,C.ColorName,SZ.SizeName,BR.Name as Brand, I.ItemName,GN.Name as GenericName,MU.UnitName,SG.SubGroupName,G.GroupName From Inventory_RStock as R
