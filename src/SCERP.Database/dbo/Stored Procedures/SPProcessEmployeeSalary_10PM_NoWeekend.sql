...
-- Description:	<> EXEC [SPProcessEmployeeSalary_10PM_NoWeekend] 1, 1, 1, -1, -1, -1, -1,'0715', 2017, 12, '2017-11-26', '2017-12-25', 'superadmin'   (FULL MONTH PROCESS)
-- ====================================================================================================================================

CREATE PROCEDURE [dbo].[SPProcessEmployeeSalary_10PM_NoWeekend]
...
BEGIN
		  ...
		  BEGIN TRAN
			...
			DELETE FROM [dbo].[EmployeeProcessedSalary_10PM_NoWeekend]
			WHERE           
			(
		  ...
		  INSERT INTO [dbo].[EmployeeProcessedSalary_10PM_NoWeekend]
           ([EmployeeId]
           ,[EmployeeCardId]
           ,[Name]
           ,[NameInBengali]
...
SELECT  ejc.EmployeeId
	   ...
	   ,CASE WHEN ((ISNULL(ejc.TotalExtraOTHours, 0.0)) >= (ISNULL(ejc.TotalPenaltyOTHours, 0.0))) 
	    ...
	    FROM EmployeeJobCard_10PM_NoWeekend ejc
		WHERE ejc.[Year] = @Year
		AND ejc.[Month] = @Month
		...
		AND ejc.EmployeeId NOT IN (SELECT EmployeeId FROM PayrollExcludedEmployeeFromSalaryProcess eesp
			...
			UPDATE EmployeeProcessedSalary_10PM_NoWeekend
