...
-- Full Company : <>  EXEC [SPProcessEmployeeInOutOneClick2] 1, 1, NULL, NULL, NULL, NULL, '2020-01-01','2020-01-05', 0, 0, NULL, '','superadmin'

-- ==========================================================================================================================================

CREATE PROCEDURE [dbo].[SPProcessEmployeeInOutOneClick2]
	   @CompanyId INT = NULL,
	   @BranchId INT = NULL,
...
BEGIN
		...
		UPDATE tmpEmployee SET Othours = 0, ExtraOtHours = 0 WHERE Status IN('Leave','OSD','Holiday','Weekend')	
		
		UPDATE 	tmpEmployee SET Othours = 0 WHERE OTHours = .5
		...
		UPDATE EmployeeInOut SET TotalContinuousAbsentDays = isnull(( SELECT TOP (1) TotalContinuousAbsentDays + 1 AS TCAbs 
		FROM EmployeeInOut AS A WHERE EmployeeCardId = EmployeeInOut.EmployeeCardId AND TransactionDate < CAST(@ToDate AS DATE) AND 
		(Status NOT IN ('Weekend')) ORDER BY TransactionDate DESC),0) WHERE TransactionDate = CAST(@ToDate AS DATE) AND Status = 'Absent'

		UPDATE EmployeeInOut SET TotalContinuousAbsentDays = 10 WHERE TransactionDate = CAST(@ToDate AS DATE) AND Status = 'Absent' and TotalContinuousAbsentDays > 10
