...
-- Description:	<> EXEC [SPProcessBulkEmployeeJobCard] -1, -1, -1, -1, -1, -1, '2015-10-01','2015-10-31', -1, '0985','superadmin'
-- ============================================================================================================================

CREATE PROCEDURE [dbo].[SPProcessBulkEmployeeJobCard]
			...
			BEGIN
				INSERT INTO [dbo].[EmployeeJobCard]
				   ...
				   ,[IsActive])
					...
					,CONVERT(DECIMAL(18,2), ((employeeSalary.BasicSalary /(DATEDIFF(DAY, @FromDate, @ToDate) + 1 )) * (dbo.fnGetIndividualLeaveDays(Employee.EmployeeId, @FromDate, @ToDate, 6))))
					,CONVERT(DECIMAL(18,2), (((employeeSalary.BasicSalary /30) * (dbo.fnGetAbsentDays(Employee.EmployeeId, @FromDate, @ToDate))))) -- + ((employeeSalary.GrossSalary /30) * 5))) --- For November'2015 (25 Days Salary).Gross salary of 5 days will be added to absent fee. It will be changed from december'2015.
					...
					,CONVERT(DECIMAL(18,2),(dbo.fnGetTotalExtraOTHours(Employee.EmployeeId, @fromDate, @toDate)))
					,CONVERT(DECIMAL(18,2),(dbo.fnGetTotalWeekendOTHours(Employee.EmployeeId, @fromDate, @toDate)))
					,CONVERT(DECIMAL(18,2),(dbo.fnGetTotalHolidayOTHours(Employee.EmployeeId, @fromDate, @toDate)))
					...
					,dbo.fnGetEmployeeTotalPenaltyOTHours(employee.EmployeeId, employee.JoiningDate, employee.QuitDate, @fromDate, @toDate) 
					,CONVERT(DECIMAL(18,2), (CAST((((employeeSalary.BasicSalary/208.00)* (dbo.fnGetOverTimeRate(@fromDate, @toDate))) 
								* (dbo.fnGetEmployeeTotalPenaltyOTHours(employee.EmployeeId, employee.JoiningDate, employee.QuitDate, @fromDate, @toDate))) AS DECIMAL(18,2)))) 
