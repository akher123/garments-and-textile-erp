CREATE procedure [dbo].[spDyeingFinishFabricReceivableBills]
...
where CAST(FI.ChallanDate AS DATE)=CAST(@InvoiceDate AS DATE)
group by FID.BatchDetailId, P.Name ,
FI.FinishFabIssureRefId ,
FI.ChallanNo,
FI.ChallanDate,P.DglId) AS ST
group by RefId,ST.AccountName,ST.DglId,ST.InvoiceDate,ST.InvoiceNo,ST.Party
