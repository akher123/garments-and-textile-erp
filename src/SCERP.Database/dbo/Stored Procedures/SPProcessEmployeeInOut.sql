...
-- Description:	<> EXEC [SPProcessEmployeeInOut] 1, 1, 3, NULL, NULL, NULL, '2019-05-08','2019-05-12', 0, 0, NULL, '','superadmin'
-- ==========================================================================================================================================

CREATE PROCEDURE [dbo].[SPProcessEmployeeInOut]
	   @CompanyId INT = NULL,
	   @BranchId INT = NULL,
		...
		BEGIN	
			...
			BEGIN
				...
				SET  [LateInMinutes] = dbo.fnGetEmployeeLateTime(@EmployeeIdTemp, @ShiftDateTemp, @EmployeeWorkShiftIdTemp, @JoiningDateTemp, @QuitDateTemp)
					...
					,[LastDayOutTime] = dbo.fnGetEmployeeLastDayOutTime(@EmployeeIdTemp, @ShiftDateTemp) -- can be omit					
				...
				UPDATE [EmployeeInOut]
				SET  [TotalContinuousAbsentDays] = dbo.fnGetTotalContinuousAbsentDays(@EmployeeIdTemp, @ShiftDateTemp, 0, @EmployeeWorkShiftIdTemp)	 -- can be omit
				WHERE [Id] = @EmployeeInOutId
				...
				UPDATE [EmployeeInOut]
				SET  [LastDayOTHours] = dbo.fnGetLastDayOTHours(@EmployeeIdTemp, @ShiftDateTemp)	-- can be omit
				WHERE [Id] = @EmployeeInOutId
				...
				UPDATE [EmployeeInOut]
				SET  [LastDayExtraOTHours] = dbo.fnGetLastDayExtraOTHours(@EmployeeIdTemp, @ShiftDateTemp)	 -- can be omit
				WHERE [Id] = @EmployeeInOutId
				...
				--UPDATE EmployeeInOut	-- Half Hour Rounding
				--SET OTHours = FLOOR(OTHours)
				--   ,ExtraOTHours = FLOOR(ExtraOTHours)
				--   ,OutTime = DATEADD(MINUTE, CAST(ABS(CHECKSUM(NEWID()) % 4) + 3 AS INT), CAST(DATEADD(HOUR, DATEDIFF(HOUR, 0, OutTime), 0) AS TIME))
				--WHERE [Id] = @EmployeeInOutId
				--AND FLOOR(OTHours) <> OTHours OR FLOOR(ExtraOTHours) <> ExtraOTHours

				--UPDATE EmployeeInOut
				--SET InTime = NULL
				--   ,OutTime = NULL
				--   ,LateInMinutes = NULL
				--WHERE [Id] = @EmployeeInOutId AND DATENAME(DW, TransactionDate) = 'Friday' AND BranchUnitWorkShiftId = 48 AND BranchUnitId <> 3
