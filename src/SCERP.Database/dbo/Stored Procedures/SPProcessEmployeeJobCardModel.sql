...
-- Description:	<> EXEC dbo.[SPProcessEmployeeJobCardModel] 1, 1, -1, -1, -1, -1, -1, '4684', 2020, 06,'2020-05-26','2020-06-25','superadmin'   (Full Month)
-- ============================================================================================================================================

CREATE PROCEDURE [dbo].[SPProcessEmployeeJobCardModel]	   				
				@CompanyId INT = -1,
				@BranchId INT = -1,
				@BranchUnitId INT = -1,
...
BEGIN
		    ...
		    BEGIN
				...
				AND ((employee.[Status] = 1) OR 
					...
					((employee.[Status] = 2) AND (employee.QuitDate > @ToDate) AND (employee.QuitDate <= DATEADD(DAY, 3000, @ToDate)))) 
