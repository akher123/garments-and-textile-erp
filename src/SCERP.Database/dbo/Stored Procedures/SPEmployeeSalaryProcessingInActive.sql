...
-- Description:	<> exec SPEmployeeSalaryProcessingInActive '06/01/2015','06/30/2015', 2, 0, 1, NULL, 2
-- ==========================================================================================================

CREATE PROCEDURE [dbo].[SPEmployeeSalaryProcessingInActive]
...
BEGIN
		...
		Declare @FromDate Datetime = CURRENT_TIMESTAMP,
				@LocalEmployeeId uniqueidentifier = '00000000-0000-0000-0000-000000000000'
        ...
        FROM	Employee AS  employee
				...
				AND (Employee.[Status] = 2 AND CAST(Employee.QuitDate AS date) >= CAST(@StartDate AS date) AND CAST(Employee.QuitDate AS date) <= CAST( @EndDate AS date))
				...
				AND (Employee.EmployeeId = @EmployeeId OR @EmployeeId = @LocalEmployeeId)
				AND (				 
					employeeCompanyInfo.BranchUnitDepartmentId = @DepartmentId 				
				...
				BEGIN
					BEGIN TRANSACTION SalaryProcess
						EXEC SPEmployeeSalarySaveInActive @StartDate, @EndDate, @EmployeeId
						FETCH NEXT FROM ProcessSalary INTO @EmployeeId
