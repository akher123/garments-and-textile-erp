CREATE procedure [dbo].[spGetMonthlyOtherCuttingProduction]
 @YearId int ,
 @MonthId int
as
...
ISNULL(((select sum(Ratio) from PROD_LayCutting where CuttingBatchRefId=C.CuttingBatchRefId and CuttingBatchId=C.CuttingBatchId and CompId=C.CompId)*(SELECT SUM(Quantity) FROM PROD_RollCutting where CuttingBatchRefId=C.CuttingBatchRefId and CuttingBatchId=C.CuttingBatchId and CompId=C.CompId)),0) AS Total  
...
INNER JOIN OM_Color AS OMC ON C.ColorRefId=OMC.ColorRefId AND C.CompId=OMC.CompId
 ...
 --where C.MachineId=52 AND C.ComponentRefId='001' and CAST(C.CuttingDate AS DATE)>=@FromDate AND CAST( C.CuttingDate AS DATE)<=@ToDate 
 --order by C.CuttingDate
 where C.MachineId=52 AND C.ComponentRefId='001' and MONTH(C.CuttingDate)=@MonthId and YEAR(C.CuttingDate)=@YearId order by C.CuttingDate