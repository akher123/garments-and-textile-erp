...
-- Description:	<> EXEC SpHrmCuttingSectionAbsent '2016-10-21'
-- ==============================================================================

CREATE PROCEDURE [dbo].[SpHrmCuttingSectionAbsent]
	
		
								@EffectiveDate	DATETIME = '2016-10-21'
...
BEGIN
			...
			SET NOCOUNT ON;
				...
				SELECT				  Employee.EmployeeCardId	
									 ...
									 ,(
										CASE WHEN EmployeeInOut.Status = 'Absent' THEN EmployeeInOut.Status 
										     WHEN EmployeeInOut.Status = 'Weekend' AND EmployeeInOut.InTime IS NULL THEN 'Absent' 
										END) AS Present
