CREATE procedure [dbo].[SpMrcAccessoriesConsumptionDetail] 
...
where  ItemCode=M.ItemCode and ColorRefId=ISNULL( D.PColorRefId,'0000') and SizeRefId=ISNULL(  D.PSizeRefId,'0000') and GColorRefId=ISNULL(D.GColorRefId,'0000') and GSizeRefId=ISNULL(D.GSizeRefId,'0000') and CommPurchaseOrder.OrderStyleRefId=M.OrderStyleRefId),0) as BookingQty,
...
where MR.RType='P' and MR.StoreId=2 and  MR.OrderStyleRefId=M.OrderStyleRefId and I.ItemCode=M.ItemCode and MRD.ColorRefId=ISNULL(D.PColorRefId,'0000') and MRD.SizeRefId=ISNULL(D.PSizeRefId,'0000') and MRD.GSizeRefId=ISNULL(D.GSizeRefId,'0000') and MRD.FColorRefId=ISNULL(D.GColorRefId,'0000')),0) AS TotalRcvQty,
...
where MI.StoreId=2 and MID.ColorRefId=ISNULL(D.PColorRefId,'0000') and MID.SizeRefId=ISNULL(D.PSizeRefId,'0000')  and MID.GSizeRefId=ISNULL(D.GSizeRefId,'0000')  and MID.FColorRefId=ISNULL(D.GColorRefId,'0000') and I.ItemCode=M.ItemCode and MI.OrderStyleRefId=M.OrderStyleRefId),0) as ToalIssueQty
...
WHERE (D.CompId =@CompId) AND (M.OrderStyleRefId =@OrderStyleRefId) and M.ConsGroup='B' and left(I.ItemCode,2) in('03','04') 

 group by   M.ItemCode, I.ItemName,GC.ColorName,GS.SizeName, C.ColorName,S.SizeName,U.UnitName,D.Remarks,BSS.SizeRow, D.PColorRefId,D.PSizeRefId,D.GColorRefId,D.GSizeRefId,M.OrderStyleRefId
