CREATE procedure [dbo].[SpYearlyBuyerWiseOrderStatus]
as
...
from 
(
  SELECT     B.BuyerName, (YEAR(SH.ShipDate) * 100 + MONTH(SH.ShipDate)) AS xM,Convert(int, SUM(SH.Quantity-ISNULL(SH.DespatchQty,0))) AS Qty
FROM         OM_BuyOrdStyle AS OS 
inner join OM_BuyOrdShip as SH on OS.OrderStyleRefId=SH.OrderStyleRefId and OS.CompId=SH.CompId
INNER JOIN
                      OM_BuyerOrder AS O ON OS.CompId = O.CompId AND OS.OrderNo = O.OrderNo INNER JOIN
...
GROUP BY (YEAR(SH.ShipDate) * 100 + MONTH(SH.ShipDate)), B.BuyerName
having Convert(int, SUM(SH.Quantity-ISNULL(SH.DespatchQty,0)))>0
) src
pivot
(
  SUM(Qty)
