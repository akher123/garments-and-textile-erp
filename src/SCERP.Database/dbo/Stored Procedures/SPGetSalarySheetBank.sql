...
-- Description:	<> EXEC [SPGetSalarySheetBank] 1, 1, -1, -1, -1, -1, -1, '', 2017, 5, '2017-04-26', '2017-05-25', -1, 'superadmin'
-- ========================================================================================================================


CREATE PROCEDURE [dbo].[SPGetSalarySheetBank]
...
FROM					[dbo].[EmployeeProcessedSalary] eps
						...
						AND eps.EmployeeId IN (SELECT EmployeeId FROM EmployeeBankInfo WHERE IsActive = 1)

						AND (eps.QuitDate IS NULL OR CAST(eps.QuitDate AS DATE) NOT BETWEEN @fromDate AND @toDate)
