...
-- Description:	<> EXEC [SPGetExtraOTAndWeekendOTSheet] -1, -1, 1, -1, -1, -1, -1, '0014', 2016, 4,'2016-03-26','2015-04-25',1,'superadmin'
-- =========================================================================================================================================


CREATE PROCEDURE [dbo].[spPayrollGetExtraOTWeekendOTAndHolidayOTSheet]
...
BEGIN
	...
	SET NOCOUNT ON;
			...
			SELECT
				  ...
				  ,eps.[BasicSalary]
				  ,eps.[GrossSalary]
				  ,eps.[TotalExtraOTHours] 
				  ,eps.[TotalWeekendOTHours] 
				  ,eps.[TotalHolidayOTHours] 
				  ,eps.[OTRate] 
				  ,eps.[TotalExtraOTAmount] 
				  ,eps.[TotalWeekendOTAmount] 
				  ,eps.[TotalHolidayOTAmount] 
				  ,ROUND((ISNULL(eps.[TotalExtraOTAmount],0) + ISNULL(eps.[TotalWeekendOTAmount],0) + ISNULL(eps.[TotalHolidayOTAmount],0)) ,0) AS TotalAmount
				...
				WHERE 
						...
						AND ((eps.TotalExtraOTAmount > 0.0)	OR (eps.TotalWeekendOTAmount > 0.00) OR (eps.TotalHolidayOTAmount > 0.00))						
