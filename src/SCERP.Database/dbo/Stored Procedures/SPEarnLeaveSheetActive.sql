...
-- Description:	<> EXEC SPEarnLeaveSheetActive '2017-04-25', '2018-02-28', '',-1,-1
-- ===================================================================================

CREATE PROCEDURE [dbo].[SPEarnLeaveSheetActive]
...
BEGIN
							...
							, ( SELECT COUNT(1) FROM EmployeeInOut WHERE EmployeeId = Employee.EmployeeId AND (Status = 'Present' OR Status = 'Late') AND CAST(TransactionDate AS DATE) < @toDate AND IsActive = 1 ) as 	TotalPresent						
							...
							, CAST(CAST(( SELECT COUNT(1) FROM EmployeeInOut WHERE EmployeeId = Employee.EmployeeId AND (Status = 'Present' OR Status = 'Late') AND CAST(TransactionDate AS DATE) < @fromDate AND IsActive = 1 ) as DECIMAL(18,2))/18.00 as DECIMAL(18,2))  AS PreviousEarnDays														
							--,FLOOR([dbo].[fnGetEarnLeave] (Employee.EmployeeId, @fromDate, @toDate,'consumed')) AS EarnLeaveConsumed
							, isnull(( SELECT SUM(ApprovedTotalDays) FROM EmployeeLeave WHERE EmployeeId = Employee.EmployeeId AND LeaveTypeId = 5 AND ApprovalStatus = 1 AND IsActive = 1 AND CAST(ApprovedFromDate AS DATE) >= @FromDate AND CAST(ApprovedToDate AS DATE) <= @ToDate ),0) AS EarnLeaveConsumed	
							...
							WHERE  (employee.IsActive = 1)
								...
								--AND CAST(employee.QuitDate AS DATE) IS NOT NULL
								AND DATEDIFF(MONTH,CAST(employee.JoiningDate AS DATE),@toDate)>=12
