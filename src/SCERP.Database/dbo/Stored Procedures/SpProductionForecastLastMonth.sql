...
CREATE PROCEDURE [dbo].[SpProductionForecastLastMonth]
@OutputDate DateTime
AS
BEGIN
...
where BO.SCont='N'   and year(SH.ShipDate)=year(@OutputDate) and Month(SH.ShipDate)=Month(@OutputDate)),0) 
set @dayCount=(select Count(*) from PLAN_WorkingDay where DayStatus=1 and WorkingDate>=@OutputDate and month(WorkingDate)=month(@OutputDate)  and year(WorkingDate)=year(@OutputDate))
...
--exec [SpProductionForecastLastMonth] '2017-03-01'
