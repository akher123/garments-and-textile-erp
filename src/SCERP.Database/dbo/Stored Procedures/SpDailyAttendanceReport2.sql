...
-- Description:	<> EXEC SpDailyAttendanceReport2   1, 1, 1, NULL, NULL, NULL, NULL, '', '2018-07-02'
-- ===================================================================================================

CREATE PROCEDURE [dbo].[SpDailyAttendanceReport2]
								...
								WHERE	(company.Id = @CompanyId OR @CompanyId IS NULL)
								 ...
								 SELECT  Department
								    ...
								    (
										...
										FROM #TableTemp 
										WHERE Remarks = 'Present' AND LineId IS NULL
										GROUP BY SectionId
									...
									UPDATE DailyPresentReport
									SET [Percent] = CAST(CAST(Present + Late AS FLOAT) / CAST(TotalEmployee AS FLOAT) * 100 AS NUMERIC(18,2))
									...
									BEGIN
											...
											SET BudgetManPower = (SELECT ApprovedManPower FROM ManPowerApprovedBySection WHERE SectionId = 6)
											   ...
											   ,Present = (SELECT COUNT(1) FROM #TableTemp WHERE Remarks = 'Present' AND SectionId = 6)
											...
											UPDATE DailyPresentReport
												SET [Balance] = BudgetManPower - TotalEmployee
												   ,[Percent] = CAST(CAST((Present + Late) AS FLOAT)/TotalEmployee * 100 AS NUMERIC(18,2))
											WHERE LineId  = 25
									  END
									  ...
									  AS Late, SUM(Absent) AS Absent, SUM(Leave) AS Leave, 0 AS [Percent], 1 AS IsActive
									  FROM DailyPresentReport AS DailyPresentReport_1
									  WHERE (DepartmentId = 6) AND (SectionId = 6) AND LineId BETWEEN 9 AND 15
									  GROUP BY DepartmentName, DepartmentId, SectionName, SectionId
