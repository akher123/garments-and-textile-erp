...
-- Description:	<> EXEC SPEarnLeaveSheetQuitAttendence '2018-01-01', '2018-12-31', '1795',-1,-1
-- ==============================================================================================

CREATE PROCEDURE [dbo].[SPEarnLeaveSheetQuitAttendence]
...
BEGIN
                            ...
                            LEFT JOIN
							(Select EmployeeCardId,EmployeeId,[Year], isnull([1],0) as January, isnull([2],0) as February, isnull([3],0) as March, isnull([4],0) as April, isnull([5],0) as May, isnull([6],0) as June, isnull([7],0) as July, isnull([8],0) as August, isnull([9],0) as September, isnull([10],0) as October, isnull([11],0) as November, isnull([12],0) as December from  ( SELECT EmployeeCardId,EmployeeId, [Year], [MONTH] AS xm, SUM(PresentDays) AS tdays
                            FROM         EmployeeJobCard
                            WHERE      IsActive=1 GROUP BY EmployeeCardId,EmployeeId, [Year], [MONTH] --(YEAR(TransactionDate) = YEAR(@fromDate)) and IsActive=1 and Status='Present'--InTime is not null --and LateInMinutes>=15
                            )  D PIVOT (max(tdays)
							...
							WHERE  (employee.IsActive = 1)
								AND (employee.[Status] = 1 OR employee.[Status] = 2)
							...
							order by employeeAtSum.EmployeeCardId,employeeAtSum.[Year]
