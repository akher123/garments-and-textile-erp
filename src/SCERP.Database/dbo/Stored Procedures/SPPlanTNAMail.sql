...
-- Description:	<> exec SPPlanTNAMail  '001'
-- ==============================================================================

CREATE PROCEDURE [dbo].[SPPlanTNAMail]
...
BEGIN
			...
			SET NOCOUNT ON;
					...
					SELECT PLAN_TNA.Id
						  ...
						  ,PLAN_TNA.EditedBy
						  ,PLAN_TNA.IsActive						
						  ,'kallol39@gmail.com' AS EmailAddress
						  ,PLAN_TNA.NotifyBeforeDays - DATEDIFF(day, @Date, PLAN_TNA.PlannedStartDate) AS StartStatus
						  ,PLAN_TNA.NotifyBeforeDays - DATEDIFF(day, @Date, PLAN_TNA.PlannedEndDate) AS EndStatus
						  ...
						  WHERE (PLAN_TNA.NotifyBeforeDays = DATEDIFF(day, @Date, PLAN_TNA.PlannedStartDate) OR PLAN_TNA.NotifyBeforeDays = DATEDIFF(day, @Date, PLAN_TNA.PlannedEndDate) )	 					   
						  AND PLAN_TNA.CompId = @CompanyId	
						  AND PLAN_TNA.IsActive = 1
