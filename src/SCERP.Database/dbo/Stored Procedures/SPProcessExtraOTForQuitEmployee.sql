...
-- Description:	<> EXEC [SPProcessExtraOTForQuitEmployee] 1, 1, 1, NULL, NULL, NULL, NULL, NULL,'2015-09-01','2015-09-30'
-- =======================================================================================================================


CREATE PROCEDURE [dbo].[SPProcessExtraOTForQuitEmployee]
...
BEGIN
	...
	SET NOCOUNT ON;
			...
			INSERT INTO [dbo].[ProcessedExtraOT]
           ([EmployeeId]
           ,[EmployeeCardId]
           ,[Name]
		   ,[CompanyId]
           ...
           ,[ToDate])
			...
			SELECT		  Employee.EmployeeId
						 ...
						 ,departmentSection.DepartmentSectionId AS 	DepartmentSectionId	
						 ,Section.Name AS Section
						 ,departmentLine.DepartmentLineId AS DepartmentLineId
						 ,Line.Name AS Line		
						 ...
						 ,dbo.fnGetTotalExtraOTHours(Employee.EmployeeId, @StartDate, @EndDate) AS TotalExtraOTHour
						 ,CASE employeeCompanyInfo.IsEligibleForOvertime WHEN 1 THEN  CAST(((EmployeeSalary.BasicSalary/208.00) * dbo.fnGetOverTimeRate(@StartDate, @EndDate)) AS DECIMAL(18,2)) ELSE 0.00 END AS OTHourRate
						 ,(ROUND(CAST(((EmployeeSalary.BasicSalary/208.00) * (dbo.fnGetOverTimeRate(@StartDate, @EndDate))) * (dbo.fnGetTotalExtraOTHours(Employee.EmployeeId, @StartDate, @EndDate))  AS DECIMAL(18,2)),0)) AS OTAmount   						 																												
...
FROM					 Employee 
						 ...
						 WHERE      
								...
								AND CAST(employee.QuitDate AS DATE)>CAST(@StartDate AS DATE)
								AND CAST(employee.QuitDate AS DATE)<=CAST(@EndDate AS DATE)
								AND ((dbo.fnGetTotalExtraOTHours(Employee.EmployeeId, @StartDate, @EndDate)) > 0.0)
								AND (employee.EmployeeCardId IN
						('0384',
'0125',
'0779',
'0383',
