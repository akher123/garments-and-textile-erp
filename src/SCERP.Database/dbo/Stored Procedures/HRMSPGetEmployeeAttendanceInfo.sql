...
-- Description:	<> EXEC HRMSPGetEmployeeAttendanceInfo  1, 1, 3, 17, 28, -1, -1, '', -1, '2018-05-16', '2018-05-16', '', -1, -1.0, -1.0, -1.0, 'superadmin'
-- ============================================================================================================================================================

CREATE PROCEDURE [dbo].[HRMSPGetEmployeeAttendanceInfo]
				@CompanyId INT = -1,
				@BranchId INT = -1,
				@BranchUnitId INT = -1,
				...
				@ExtraOTHours NUMERIC(18,2) = 0.0,
				@WeekendOTHours NUMERIC(18,2) = 0.0,
...
BEGIN
				...
				,eio.LastDayOTHours
				,eio.ExtraOTHours
				,eio.LastDayExtraOTHours
				,eio.WeekendOTHours
				,eio.HolidayOTHours
				,eio.Remarks
				,'' AS SignatureOfEmployee

				,ISNULL((SELECT TOP(1) EmployeeInOut.TransactionDate 
					FROM EmployeeInOut 
					WHERE EmployeeInOut.EmployeeId = eio.EmployeeId 
					AND EmployeeInOut.Status IN ('Present' , 'Late')  
					AND eio.IsActive = 1 AND EmployeeInOut.IsActive = 1
					ORDER BY EmployeeInOut.TransactionDate DESC
				),GETDATE()) AS LastPresentDate
				...
				AND (ExtraOTHours > @ExtraOTHours OR @ExtraOTHours IS NULL)		
				AND (WeekendOTHours > @WeekendOTHours OR @WeekendOTHours IS NULL)	 	
