...
-- Description:	<> EXEC [SPSalaryOneYearMatureIncrementReport] 1, 1, -1, -1, -1, -1, '2017-06-26','2017-07-25', -1, '','superadmin', 5, 0
-- ==========================================================================================================================================

CREATE PROCEDURE [dbo].[SPSalaryOneYearMatureIncrementReport]
...
BEGIN
						...
						Select A.EmployeeCardId, A.EmployeeId, A.Name, CAST(JoiningDate as DATE) as JoiningDate,CAST(LastIncrementDate AS DATE) as LastIncrementDate, PreSal as PreviousSalary, PreSal*0.05 as PIncrement, PreSal*1.05 ProposedSalary, CurSal as CurrentSalary, PreSal*1.05-CurSal as ProposedIncrement From (select EmployeeCardId,EmployeeId,Name,JoiningDate, 
                         ...
                         --FROM            EmployeeSalary
                           --WHERE        (EmployeeId = Employee.EmployeeId) AND (FromDate < '2016-05-26') ) as PSal,  (SELECT        SUM(BasicSalary) AS Expr1
							...
							(select  BasicSalary from (SELECT EmployeeId, GrossSalary,  BasicSalary,HouseRent, MedicalAllowance, Conveyance,FoodAllowance,EntertainmentAllowance,FromDate,												
							  ...
							  (select  FromDate from (SELECT EmployeeId, GrossSalary,  BasicSalary,HouseRent, MedicalAllowance, Conveyance,FoodAllowance,EntertainmentAllowance,FromDate,												
							  ROW_NUMBER() OVER (PARTITION BY EmployeeId ORDER BY FromDate DESC) AS rowNum						 
							  FROM EmployeeSalary  
							  WHERE (CAST(EmployeeSalary.FromDate AS Date) <= @ToDate)) AS EmployeeSal where rowNum=1 and EmployeeId=Employee.EmployeeId) AS LastIncrementDate
