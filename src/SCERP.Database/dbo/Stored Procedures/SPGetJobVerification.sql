...
-- Description:	<> EXEC SPGetJobVerification '9BC5D1C6-0AD1-4D9E-A7F3-03CA3B155BC8', '9BC5D1C6-0AD1-4D9E-A7F3-03CA3B155BC8', '2018-04-10' 
-- ======================================================================================================================================

CREATE PROCEDURE [dbo].[SPGetJobVerification]
...
BEGIN
			...
			DECLARE @userName NVARCHAR(100) = 'superadmin'
			...
			SELECT		  Employee.EmployeeId
						 ...
						 ,employeePresentAddress.EmergencyContactPersonInBengali
						 ,employeePresentAddress.ContactPersonRelationInBengali	
						 ,employeePresentAddress.ContactPersonPhone

						 ,EmployeeVerifier.NameInBengali AS VerifierNameInBangla
						 ,EmployeeVerifier.EmployeeCardId AS VerifierCardId
						 ,employeeDesignationVerifier.TitleInBengali AS VerifierDesignation
...
FROM			    	Employee AS  employee
						...
						LEFT JOIN Employee AS EmployeeVerifier ON EmployeeVerifier.EmployeeId = @userId	
						LEFT JOIN (SELECT EmployeeId, FromDate, JobTypeId, DesignationId, BranchUnitDepartmentId, DepartmentSectionId, DepartmentLineId,
						ROW_NUMBER() OVER (PARTITION BY EmployeeId ORDER BY FromDate DESC) AS verifierRowNum
						FROM EmployeeCompanyInfo AS verifieremployeeCompanyInfo 
						WHERE ((CAST(verifieremployeeCompanyInfo.FromDate AS Date) <= @FromDate) OR (@FromDate IS NULL)) AND verifieremployeeCompanyInfo.IsActive=1) VerifieremployeeCompanyInfo 
						ON EmployeeVerifier.EmployeeId = VerifieremployeeCompanyInfo.EmployeeId 	
						INNER JOIN EmployeeDesignation AS employeeDesignationVerifier ON verifieremployeeCompanyInfo.DesignationId = employeeDesignationVerifier.Id

						WHERE employee.IsActive = 1 											 
						AND employeeCompanyInfo.rowNum = 1 
						AND VerifieremployeeCompanyInfo.verifierRowNum = 1
