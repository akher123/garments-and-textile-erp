...
-- Full Company : <>  EXEC [SPProcessEmployeeInOutOneClick]  1, 1, 1, NULL, NULL, NULL, '2019-08-04','2019-08-04', 0, 0, NULL, '','superadmin'
-- ===============================================================================================================================================

CREATE PROCEDURE [dbo].[SPProcessEmployeeInOutOneClick]
	   @CompanyId INT = NULL,
	   @BranchId INT = NULL,
...
BEGIN
		...
		UPDATE tmpEmployee SET [OutTime] = isnull(dbo.fnGetEmployeeOutTime(EmployeeId, ShiftDate, EmployeeWorkShiftId, JoiningDate, QuitDate), '00:00:00') ;		
		UPDATE tmpEmployee SET [OutTime] =NULL where [OutTime]='00:00:00';	
		...
		UPDATE tmpEmployee  SET  [TotalContinuousAbsentDays] =0;
		UPDATE tmpEmployee  SET  [TotalContinuousAbsentDays] = dbo.fnGetTotalContinuousAbsentDays(EmployeeId, ShiftDate, 0,EmployeeWorkShiftId) where Status ='ABSENT'	;
		UPDATE tmpEmployee  SET  [OTHours] = 0 WHERE CAST ([OTHours] AS FLOAT) = CAST(.5 AS FLOAT)
		
		UPDATE tmpEmployee  SET  OTHours = 2, ExtraOTHours = 4 + 1 + DATEDIFF(HOUR, CONVERT(DATETIME, '00:00:00'), CONVERT(DATETIME, outTime)) WHERE InTime IS NOT NULL AND OutTime BETWEEN CAST('00:30:00' AS TIME) AND CAST('07:00:00' AS TIME) AND tmpEmployee.EmployeeType IN ('Team Member-A' , 'Team Member-B') 
		...
		EXEC SPMISInsertSalarySummaryDaily @FromDate     -- newly added for daily CM calculation
