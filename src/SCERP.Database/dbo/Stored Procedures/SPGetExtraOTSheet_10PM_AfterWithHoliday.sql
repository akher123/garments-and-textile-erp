...
-- Description:	<> EXEC [SPGetExtraOTSheet_10PM_AfterWithHoliday] 1, 1, 1, 6, 6, 6, -1, NULL, 2020, 01,'2019-12-26','2020-01-25',1, 'superadmin'
-- ===========================================================================================================

CREATE PROCEDURE [dbo].[SPGetExtraOTSheet_10PM_AfterWithHoliday]
...
BEGIN
	...
	SET NOCOUNT ON;
			...
			SELECT
				  ...
				  ,eps.[BasicSalary]
				  ,eps.[GrossSalary]
				  ,(eps.[TotalExtraOTHours] - eps10.[TotalExtraOTHours]) AS TotalExtraOTHours
				  ,eps.[OTRate] 
				  ,CAST((eps.[TotalExtraOTHours] - eps10.[TotalExtraOTHours]) * eps.[OTRate] AS DECIMAL(18,2)) AS TotalExtraOTAmount
				  --,(eps.[TotalExtraOTAmount] - eps10.[TotalExtraOTAmount]) AS TotalExtraOTAmount
				  ,eps.[MonthName] AS   [MonthYear]
				  ,CONVERT(VARCHAR(10), eps.[FromDate], 103) AS FromDate
				  ,CONVERT(VARCHAR(10), eps.[ToDate], 103) AS ToDate
				  ,eps.TotalHolidayOTHours
				  ,eps.TotalHolidayOTAmount
			
				FROM [dbo].[EmployeeProcessedSalaryGrossDeduction] eps
				LEFT JOIN EmployeeProcessedSalary_10PM_NoWeekend AS eps10 ON eps10.EmployeeId = eps.EmployeeId AND eps10.Month = eps.Month AND eps10.Year = eps.Year AND CAST(eps10.FromDate AS DATE) = CAST(eps.FromDate AS DATE) AND CAST(eps10.ToDate AS DATE) = CAST(eps.ToDate AS DATE)
				...
				WHERE 
						...
						AND eps.TotalExtraOTAmount > 0.0
						
						AND (eps.[TotalExtraOTHours] - eps10.[TotalExtraOTHours]) > 0

						--AND eps.EmployeeCardId = 8603
