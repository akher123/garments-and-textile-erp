...
{
    public class ProjectDocumentInfoManager : BaseManager, IProjectDocumentInfoManager
    {
        ...
        public bool CheckExistingDocumentationReport(CRMDocumentationReport documentationReport)
        {
            ...
            try
            {
                isExist = _documentationReportRepository.Exists(x => x.IsActive && x.Id != documentationReport.Id && x.ReportName.Replace(" ", "").ToLower().Equals(documentationReport.ReportName.Replace(" ", "").ToLower()));
            }
            catch (Exception exception)
            {
                Errorlog.WriteLog(exception);
